...
class GameState {
    constructor() {
        ...
        // Check for old balance system and migrate it
        const oldBalance = localStorage.getItem('balance');
        if (oldBalance && !isNaN(parseInt(oldBalance))) {
            const parsedBalance = parseInt(oldBalance);
            if (parsedBalance > this.state.balance) {
                this.state.balance = parsedBalance;
                this.saveState();
            }
        }
        
        // Initialize main balance display
    ...
    notifyObservers() {
        ...
        // Always update main page balance regardless of game state
    ...
    setItemActive(itemName, active) {
        if (this.state.items[itemName]) {
            this.state.items[itemName].active = active === true;
            this.saveState();
            this.notifyObservers();
        }
    }
    ...
    isItemActive(itemName) {
        return this.state.items[itemName]?.active === true;
    }
...
// Update main balance display when DOM is ready
